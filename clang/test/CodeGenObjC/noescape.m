...
// CHECK: declare void @noescapeFunc0(ptr noundef, {{.*}} noundef captures(address))
...
// CHECK: define internal void @"\01-[C0 m0:]"({{.*}}, {{.*}}, {{.*}} captures(address) {{.*}})

// CHECK-LABEL: define{{.*}} void @test4(
// CHECK: call void @objc_msgSend(ptr {{.*}}, ptr {{.*}}, ptr noundef captures(address) {{.*}})
...
// CHECK: define internal void @{{.*}}(ptr {{.*}}, ptr noundef captures(address) {{.*}})
